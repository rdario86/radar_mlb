...
if st.session_state.df_mlb is not None:
    ...
    with tab6:
        ...
        st.markdown("* **📉 El Candado de Ases (Poisson >= 60% + IP >= 5.0):** El lanzador recibe la estrella premium (⭐) solo si la distribución matemática le otorga un 60% de cubrir su línea de 'Over', y además promedia un volumen de al menos **5.0 innings lanzados por salida**. Esto permite detectar un volumen consistente de abridores dominantes con alto K/9, eliminando a relevistas largos o lanzadores con restricciones de pitcheos.")
