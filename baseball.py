...
def get_hrr_hunters(fecha_hoy):
    try:
        ...
        for r in top_4:
            ...
            # =========================================================
            # 🛡️ MODO FRANCOTIRADOR: >= 72% Poisson Y >= 4.3 Avg PA
            # =========================================================
            if r["📉 Probabilidad"] >= 72 and float(r["📊 Avg PA (L7)"]) >= 4.3:
                es_alta_seg = True
...
if st.session_state.df_mlb is not None:
    ...
    with tab7:
        ...
        st.markdown("**⭐ Selección Premium (Modo Francotirador):**")
        st.markdown("* **📉 Doble Candado Extremo (Poisson >= 72% + PA >= 4.3):** El jugador recibe la estrella premium (⭐) solo si las matemáticas le otorgan un brutal 72% de cubrir la línea **Y** promedia al menos 4.3 apariciones al plato (PA). Esto reduce el volumen a máximo 4-5 selecciones semanales, garantizando que el dinero solo respalda a los tres primeros bates de la alineación que se encuentran en rachas históricas ante pitcheos vulnerables.")
