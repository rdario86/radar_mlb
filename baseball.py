...
def get_strikeout_hunters(fecha_hoy):
    try:
        ...
        for r in top_4:
            ...
            # 🌟 FILTRO DINÁMICO DE ESTRELLA (PROB >= 71%) - Sin restricciones de IP
            if r["prob_pct"] >= 71: # <--- Ajustado a 71%
                es_alta_seg = True
...
if st.session_state.df_mlb is not None:
    ...
    with tab7:
        ...
        st.markdown("* **📉 Probabilidad de Alta Seguridad (>=71%):** El modelo aprovecha el colchón matemático para exigir un margen de seguridad sólido en bajas.")
        
        st.markdown("**⭐ Para las jugadas OVER:**")
        st.markdown("* **📈 Probabilidad de Alta Seguridad (>=71%):** La línea asignada es conservadora, permitiendo que el pitcher se quede un ponche corto de su proyección y aún así exigir una alta probabilidad estadística.")
